%MDS Trainable mapping for multidimensional scaling, a variant of Sammon mapping 
%
...
%   OPTIONS Various parameters of the  minimisation procedure put into 
...
% optimisation procedures minimizing the stress for the test objects, while
  ...
  %  no projection is available yet.
    ...
    if (isdataset(D) | isa(D,'double'))
      ...
      if (ismapping(y))
        ...
        % Initialisation by 'cs' is not possible when there is no winit 
        % (i.e. the CS map) and new points should be added.
        if (strcmp(opt.init,'cs')) & (isempty(winit))
          prwarning(3,'OPTIONS.init = cs is not possible when adding points; using kl.');
          opt.init = 'kl';  
        end
        ...
        if (length(pars) < 6) | (isempty(pars{6}))
          ...
          if (size(yinit,1) ~= size(D,1))
            prwarning(2,'the size of the initial configuration does not match that of the dissimilarity matrix, using random initialisation.')
            yinit =[];
          end
        end
        ...
        if (~issym(D,1e-12))
          prwarning(3,'D is not a symmetric matrix; will average.'); 
          D = (D+D')/2;
        end
...
function printinfo(opt)
  ...
  switch (opt.optim)
    case 'pn',  
      %fprintf(opt.st,'Minimisation by Pseudo-Newton algorithm\n');
    case 'scg',  
      %fprintf(opt.st,'Minimisation by Scaled Conjugate Gradients algorithm\n');
    otherwise 
      error(strcat('Possible initialisation methods: pn (Pseudo-Newton), ',...
...
% minimisation of a variant of the Sammon stress. The minimisation starts
...
function [y,err] = mds_sammap(Ds,y,yrep,opt)
  ...
  switch (opt.optim)  
    case 'pn',        % Pseudo-Newton minimisation.
      ...
      while (abs(e-eold) >= opt.etol*(1 + abs(e))) & (it < opt.maxiter)      
        if runtime > prtime
          prwarning(2,'optimisation stopped by PRTIME');
          break;
        end
        ...
        prwaitbar(prtime,runtime,[tt num2str(it) ', error: ' num2str(e)]);
        %if opt.st > 0, fprintf(opt.st,'iteration: %4i   stress: %3.8d\n',it,e); end
        runtime = etime(clock,starttime);

      end

    case 'scg',              % Scaled Conjugate Gradient minimisation.

      sigma0  = 1e-8;                        
      lambda  = 1e-8;       % Regularisation parameter.
      lambda1 = 0;                                                             
      ...
      if (gnorm2 < 1e-15)
        prwarning(3,['Gradient is nearly zero: ' gnorm2 ' (unlikely); initial configuration is the right one.']);  
        return
      end
      ...
      while (abs(eold - e) > opt.etol * (1 + e)) & (it < opt.maxiter)   
        ...
        % Regularise the Hessian indirectly to make it positive definite.
        % DELTA is now computed as P'* (Hessian + regularisation * Identity) * P
        delta = delta + (lambda1 - lambda) * pnorm2;  

        % Indicate if the Hessian is negative definite; the regularisation above was too small.
        if (delta < 0)
        ...
        % This minimisation procedure is based on the second order approximation of the  
        % stress function by using the gradient and the Hessian approximation. The Hessian 
        % is regularised, but maybe not sufficiently. The ratio Dc (<=1) below indicates 
        ...
        if (success)
          ...
          if (Dc >= 0.75)
            lambda = 0.25 * lambda;             % Make the regularisation smaller
          end

          it  = it + 1; 
          err = [err; e];
          %prwaitbar(prwaitbar,runtime,[tt num2str(it) ', error: ' num2str(e)]);
          fprintf (opt.st,'iteration: %4i   stress: %3.8d\n',it,e); 

        else        % Dc < 0  
          % Note that for Dc < 0, the iteration number IT is not increased, 
          % so the Hessian is further regularised until SUCCESS equals 1.
          lambda1 = lambda;
        end

      % The approximation of the Hessian was poor or the stress was not 
      % decreased (Dc < 0), hence the regularisation lambda is enlarged.
      if (Dc < 0.25)
...
%MDS_STRESS - Calculate Sammon stress during optimisation
%
