...
%
% Be aware that in case A is not a dataset but an array of doubles, A*W is
% also an array of doubles and the output of A*W*LABELD is a column vector 
% pointing to the column with maximum classification confidence. If the
% real labels strored in W are needed, convert A to a dataset first.
%
  ...
  [a,w,thresh] = deal(argin{:});
    ...
    if isempty(featlist)
      ...
      labels = featlist(J,:);
		...
		labels = feval(mfilename,prdataset(a)*w,thresh);
