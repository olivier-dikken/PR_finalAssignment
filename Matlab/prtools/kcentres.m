...
%   N    Number of trials starting from a random initialisation
%        (optional; default: 1)
%
% OUTPUT
...
% distances over all objects to the nearest center is minimised. For K > 1,
% the results depend on a random initialisation. The procedure is repeated 
...
function [labels,Jopt,dm] = kcentres(d,k,n)
	...
	if (nargin < 3) | isempty(n)
      n = 1;
      prwarning(3,'Number of trials not supplied, assuming one.');
   end

	if (nargin < 2) | isempty(k)
      k = 1;
      prwarning(3,'Number of centers not supplied, assuming one.');
   end
	
	if(isdataset(d))
		d = +d;
		prwarning(3,'Distance matrix is convert to double.');
	end
...
% checking for a zero diagonal 
	...
	for tri = 1:nrep
		...
		if n == 0
			...
			M = randperm(m); M = M(1:k);		% Random initialisations
		end
...
% to the first K: max(min(D(:,N(1:K),[],2)) is minimised.
%
