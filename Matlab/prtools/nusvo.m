%NUSVO Support Vector Optimiser: NU algorithm, low-level routine
%
...
%                           an optimisation failure and treated accordingly (deafault: 1)
...
%    .PF_ON_FAILURE         if the optimisation is failed (optimiser did not converge, or there are 
...
%
% DESCRIPTION
% A low level routine that optimises the set of support vectors for a 2-class
...
% the two classes by +1 and -1. Optimisation is done by a quadratic programming. 
...
function [v,J,nu,C] = nusvo(K,y,nu,Options,arg)
  ...
  if nargin < 3
		prwarning(3,'The regularisation parameter NU is not specified, assuming 0.01.');
    nu = 0.01;
	end
  ...
  if Options.pd_check
  	...
  	if (i > -30),
	  	prwarning(3,'K is not positive definite. The kernel is regularised by adding 10.0^(%d)*I',i);
  	end
	  i = i+2;
  	D = D + (10.0^(i)) * eye(n);
	end
   
  % Minimisation procedure initialisation:
	% 'qp' minimises:   0.5 x' D x + f' x
	% subject to:       Ax <= b
	%
  if (exist('qld') == 3) 
		%prwarning(2,'PRTools'' QLD is used for SVM optimisation.')
		v = qld (D, f, -A, b, lb, ub, p, length(b));
  elseif (exist('quadprog') == 2)
		prwarning(2,'The Matlab routine QUADPROG is used for SVM optimisation.')
    options = optimoptions(@quadprog);
    options.Display = 'off';
		v = quadprog(D, f, [], [], A, b, lb, ub,[],options);
  elseif (exist('qp') == 2)
		prwarning(2,'The Matlab routine QP is used for SVM optimisation.')
		verbos = 0;
		negdef = 0;
		normalize = 1;
		v = qp(D, f, A, b, lb, ub, p, length(b), verbos, negdef, normalize);
  else
    error('No optimiser found for PRTools SVC')
  end
  
  try
    % check if the optimiser returned anything
    if isempty(v)
      error('prtools:nusvo','Optimisation did not converge.');
    end
    ...
    if Options.pf_on_failure
      prwarning(2,'optimisation failed, pseudo-inverse is used instead.');
      n = size(K,1);
...
function [rho, b] = FindRhoAndB(nu,y,K,v,J,Jp,Jm,Ip,Im,Options)
  ...
  if ~isempty(Ip) & ~isempty(Im)
    ...
    elseif ~isempty(Ip) & isempty(Im)
      ...
      % we need to minimise(2*mSVm - nu*n)*r2
    ...
    elseif isempty(Ip) & ~isempty(Im)
      ...
      % we need to minimise(2*mSVp - nu*n)*r1
