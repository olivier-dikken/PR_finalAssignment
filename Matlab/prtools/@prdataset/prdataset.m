...
function varargout = prdataset(a,labels,varargin)
  ...
  if isempty(PRTOOLS_START)
    ...
    if isdeployed
      ...
      %                 ' (' z.Date ').'])
      disp([prnewline '   Welcome to PRTools5. It is not fully compatible with PRTools4.'])
      disp(['   Go to http://37steps.com/prtools5-intro/ for transition notes' ...
        ' or click <a href="http://37steps.com/prtools5-intro/">here</a>.'])
      try % see whether we can find the most recent version
        ver_old = prversion;
        ver_new = prversion('http://prtools.org/files/prtools_version.txt');
        if ~isempty(ver_new)
          if ver_new > ver_old
      ...
      disp([prtools_mod prnewline]) % Display message of the day
    end
  end
  ...
  if isempty(b.labtype)   % if labtype not yet set, do it now
    if isempty(labels)    % dataset without labels and targets, make crisp
      b.labtype = 'crisp';
    elseif (size(labels,2) == 1 & ~isa(labels,'prdataset')) | isstr(labels)
