...
function w = treec(varargin)
  ...
  elseif mapping_task(argin,'training')			% Train a mapping.
    ...
    if isnan(crit) & isnan(prune)        % optimise criterion and pruning, grid search
      global REGOPT_OPTCRIT REGOPT_PARS
      for n = 1:3
        defs = {n,0};
        v = regoptc(a,mfilename,{crit,prune},defs,[2],parmin_max,[],[0,0]);
    ...
    elseif isnan(crit)                    % optimise criterion 
      defs = {1,0};
      w = regoptc(a,mfilename,{crit,prune},defs,[1],parmin_max,[],[0,0]);
      
    elseif isnan(prune)                    % optimise pruning
      defs = {1,0};
      w = regoptc(a,mfilename,{crit,prune},defs,[2],parmin_max,[],[0,0]);
...
function tree = maketree(a,nlab,c,crit,stop) 
	...
	if all([nlab == nlab(1)]) 
		% Avoid giving 0-1 probabilities, but 'regularise' them a bit using
