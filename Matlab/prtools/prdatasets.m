...
if exist('sonar','file') ~= 2
  ...
  if exist(fullfile(toolsdir,'prdatasets/Contents.m'),'file') ~= 2
    ...
    if ~isempty(path)
      ...
      [ss,dirname] = prdownload('http://prtools.tudelft.nl/prdatasets5/prdatasets.zip','prdatasets');
      addpath(dirname)
    end
  else
...
elseif ~isempty(dset) & nargin == 1 % check / load m-file
	...
	if strcmpi(dset,'renew')
		if exist('prdatasets/Contents','file') ~= 2
			...
			prdownload('http://prtools.tudelft.nl/prdatasets5/prdatasets.zip',dirname);
    end
  elseif strcmpi(dset,'all')
	...
	% naming
	rootdir = fileparts(which(callername));
