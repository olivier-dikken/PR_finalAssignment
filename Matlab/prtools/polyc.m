...
function W = polyc(a,classf,n,s)
	...
	if nargin < 4, 
		prwarning(3,'S is not specified, assuming 0, i.e. no higher order terms.'); 	
		s = 0; 
	end
	if nargin < 3 | isempty(n), 
		prwarning(3,'N, the degree of polynomial is not specified, assuming 1.'); 	
		n = 1; 
	end
	if (nargin < 2) | (isempty(classf)), 
		prwarning(3,'CLASSF is not specified, assuming FISHERC.'); 	
		classf = fisherc; 
	end
