...
elseif nargin == 1
	...
	if isstr(n) & strcmpi(n,'off')           % ---- call: prwaitbar off ----
	...
	elseif isstr(n) & strcmpi(n,'on')        % ---- call: prwaitbar on ----
		STAT = 1;                             % restart waitbar
		REP = false;
    DEPTH = [];
		return
		
  elseif isstr(n) & strcmpi(n,'report')    % ---- call: prwaitbar report ----
		prwaitbar off;
    err.message = '##';
    lasterror(err);
		STAT = 1;                            
  ...
  elseif isstr(n) && strcmpi(n,'hide')      % ---- call: prwaitbar hide ----
    set(WHANDLE,'Visible','off')
    return
    
  elseif isstr(n)
    error('Unknown command for prwaitbar')
...
elseif nargin == 2
	...
	if ischar(m)                           % ---- call: prwaitbar(n,text) ----
...
if decrease_depth                         % update admin
    ...
    if REP                                % REP might be non-existent !!
      ;
    else                                  % so then we need to clean
      ...
      if isempty(p) %DXD: this happens in Matlab R2014b...
         %delete(WHANDLE);
    ...
    lasterror('reset'); % needed to have a proper restart of the waitbar
	else
		if significant1
...
function s = wtext(MESS,N,DEPTH,REP,decrease_depth)
