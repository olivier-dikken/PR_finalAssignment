...
% the class prior probabilities. So if the prior probability of a class is
...
function [A,B,IA,IB] = gendat(varargin)
  ...
  % execution
  [X,N,seed] = deal(argin{:});
  %RD: preserving seed is not desired as it reproduces random sampling
  %seednow = randreset; 
  if isempty(seed)
    seed = randreset;
  end
	...
	if (nargin < 2) || (isempty(N))			% Bootstrap
		...
		if ~islabtype(X,'crisp')
			...
			if (N > m)
				...
				prwarning(2,'More objects requested than available.')
				%N = m;
        ...
        if (N(i) > R(i))
					%DXD: I would like to have just a warning:
					%error('More objects requested than available.')
					prwarning(2,'More objects requested than available in class %d.',i)
					%N(i) = R(i);
        end
        if (~isempty(K)) && (N(i) > 0)
